...
int main(int argc, const char** argv)
{
    ...
    if (read_CMDLINE_Params(number_of_mappers, number_of_reducers, input_file,
            argv)
        == END_FUNCTION_ERROR_RETURN) {
        END_FUNCTION_ERROR
    }
    ...
    for (int i = 0; i < total_threads; i++) {
        if (i < number_of_mappers) {
            ...
            if (pthread_create(&mappers_threads[i], NULL, executeTaskMapper,
                    (void*)&myMapperTasks[i])
                != 0) {
